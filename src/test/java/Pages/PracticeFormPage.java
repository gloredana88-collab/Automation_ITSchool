...
public class PracticeFormPage {
    WebDriver driver;
    Elements_Methods elementsMetods;

    public PracticeFormPage (WebDriver driver) {
        this.driver = driver;
        this.elementsMetods = new Elements_Methods(driver);
    ...
    public void completeFirstRegionField(PracticeFormObject data){
        elementsMetods.fillText(firstNameField, data.getFirstName());
        elementsMetods.fillText(lastNameField, data.getLastName());
        elementsMetods.fillText(userEmailField, data.getEmailAddress());
        elementsMetods.fillText(currentAddressField, data.getCurrent());
        elementsMetods.fillText(userNumber, data.getPhoneNumber());
        elementsMetods.fillText(subjectField, data.getSubjects());
    }
...
//    }
    ...
    public void completeStatesAndCity(PracticeFormObject data){
        elementsMetods.clickElement(stateField);
        elementsMetods.enterTextUsingActions(stateField, data.getState());
        elementsMetods.clickElement(cityField);
...
//        elementsMetods.clickElement(calendarInput);
...
//        //"//div[@class='react-datepickerday react-datepickerday--0" + data.getDay() + "' and not(contains(@class, 'react-datepicker__day--outside-month'))]"
//        elementsMetods.clickElement(day);
//    }

        public void completeCalendar (PracticeFormObject data){
            elementsMetods.clickElement(calendarInput);
            //elementsMetods.selectByText(monthSelect,data.getMonth());
            elementsMetods.selectByValue(monthSelect,data.getMonth());
            elementsMetods.selectByValue(yearSelect,data.getYear());
